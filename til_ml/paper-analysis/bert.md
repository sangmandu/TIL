...
* 자세히 말하면 SQuAD 1.1에서 모델들이 높은 성적을 냈을 때 이에 대한 설명을 SQuAD에서는 확인할 수 없었다. 최신 리더보드 들은 이러한 정보를 제공해주는데에 비해 여기서는 이런 description을 제공해주지 않았던 것. 그래서 top 모델들이 어떤 데이터셋을 사용했는지 알 수가 없었고 이 task는 자유롭게 모든 공공 데이터를 학습할 수 있었으므로 자기들도 triviaQA 데이터를 적당히 사용했다 라는 점을 말한다. \(외부 데이터를 많이 써서 학습을 했다면 모델의 우수함보다 데이터셋의 차별성이 눈에 띄니 이 부분을 작게하기 위해 적당히 사용했다는 것을 말하는 듯\)
